import 'package:flutter/material.dart';

// ─────────────────────────────────────────────────────────────────────────────
// Shared text styles for consent content
// ─────────────────────────────────────────────────────────────────────────────

const _kHeadingStyle = TextStyle(
  fontWeight: FontWeight.w700,
  fontSize: 15,
  color: Color(0xFF0B28D9),
  height: 1.4,
);

const _kBodyStyle = TextStyle(
  fontSize: 14.5,
  color: Color(0xFF374151),
  height: 1.65,
  fontWeight: FontWeight.w400,
);

const _kBulletStyle = TextStyle(
  fontSize: 14.5,
  color: Color(0xFF374151),
  height: 1.65,
  fontWeight: FontWeight.w400,
);

const _kEmphasisStyle = TextStyle(
  fontSize: 14.5,
  color: Color(0xFF1A1A2E),
  height: 1.65,
  fontWeight: FontWeight.w600,
);

TextSpan _heading(String text) =>
    TextSpan(text: '$text\n', style: _kHeadingStyle);

TextSpan _body(String text) => TextSpan(text: text, style: _kBodyStyle);

TextSpan _bullet(String text) =>
    TextSpan(text: '  •  $text\n', style: _kBulletStyle);

TextSpan _em(String text) => TextSpan(text: text, style: _kEmphasisStyle);

TextSpan _spacer() => const TextSpan(text: '\n');

// ─────────────────────────────────────────────────────────────────────────────
// Step 1 — What This Study Is About
// ─────────────────────────────────────────────────────────────────────────────

const List<InlineSpan> kConsentStudyAboutSpans = [
  TextSpan(text: '1. What is this study about?\n', style: _kHeadingStyle),
  TextSpan(
    text:
        'You are invited to participate in a research study developing and '
        'evaluating HATI, a mobile application designed to help students who '
        'experience high levels of social anxiety symptoms. The app includes a '
        'virtual companion named "Hati" that provides gamified social scenarios, '
        'emotion tracking, and coping strategies in a safe, private environment.\n\n'
        'This study aims to understand whether such an application can help '
        'students better recognise their emotional patterns and practise coping '
        'skills in social situations.\n',
    style: _kBodyStyle,
  ),
  TextSpan(text: '\n'),
  TextSpan(
    text: '2. Why am I being invited to participate?\n',
    style: _kHeadingStyle,
  ),
  TextSpan(text: 'You are being invited because:\n', style: _kBodyStyle),
  TextSpan(
    text:
        '  •  You are a third-year undergraduate student at West Visayas State University.\n',
    style: _kBulletStyle,
  ),
  TextSpan(
    text:
        '  •  You have scored above 40 on the Social Phobia Inventory (SPIN) screening, indicating high social anxiety symptoms.\n',
    style: _kBulletStyle,
  ),
  TextSpan(
    text:
        '  •  You are not currently diagnosed with Social Anxiety Disorder or receiving active professional mental health treatment.\n',
    style: _kBulletStyle,
  ),
  TextSpan(
    text: '  •  You are 20 years of age or older.\n',
    style: _kBulletStyle,
  ),
  TextSpan(text: '\n'),
  TextSpan(
    text: '3. What will happen if I agree to participate?\n',
    style: _kHeadingStyle,
  ),
  TextSpan(
    text: 'If you agree to participate, you will be asked to:\n',
    style: _kBodyStyle,
  ),
  TextSpan(
    text:
        '  •  Install the application on your mobile device with the assistance of the researchers.\n',
    style: _kBulletStyle,
  ),
  TextSpan(
    text:
        '  •  Complete an initial SPIN assessment within the app (if not already completed).\n',
    style: _kBulletStyle,
  ),
  TextSpan(
    text: '  •  Use the HATI application for a period of two weeks.\n',
    style: _kBulletStyle,
  ),
  TextSpan(
    text:
        '  •  During use, engage with gamified social scenarios where you will type responses. You may also optionally provide brief voice recordings to help the app detect emotional cues.\n',
    style: _kBulletStyle,
  ),
  TextSpan(
    text:
        '  •  The app will track your emotional patterns over time and provide adaptive feedback from the virtual companion.\n',
    style: _kBulletStyle,
  ),
  TextSpan(
    text:
        '  •  At the end of the study period, complete another SPIN assessment and a GAD-7 assessment.\n',
    style: _kBulletStyle,
  ),
  TextSpan(
    text: '  •  Share your experience with the app through a feedback form.\n',
    style: _kBulletStyle,
  ),
  TextSpan(text: '\n'),
  TextSpan(
    text: '4. Is HATI a substitute for professional mental health treatment?\n',
    style: _kHeadingStyle,
  ),
  TextSpan(
    text:
        'No. HATI is a self-help support tool designed for non-clinical use. '
        'It is not a diagnostic instrument and does not replace professional '
        'psychological or psychiatric treatment.\n\n'
        'If you have concerns about your mental health, you may reach out to the '
        'WVSU University Guidance and Counseling Office:\n',
    style: _kBodyStyle,
  ),
  TextSpan(
    text:
        '  •  Location: Hometel, Ground Floor, West Visayas State University\n',
    style: _kBulletStyle,
  ),
];

// ─────────────────────────────────────────────────────────────────────────────
// Step 2 — What Data We Collect
// ─────────────────────────────────────────────────────────────────────────────

const List<InlineSpan> kConsentDataCollectedSpans = [
  TextSpan(
    text: '5. What type of data will be collected?\n',
    style: _kHeadingStyle,
  ),
  TextSpan(text: 'The following data will be collected:\n', style: _kBodyStyle),
  TextSpan(
    text: '  •  Assessment Data: Your SPIN and GAD-7 scores.\n',
    style: _kBulletStyle,
  ),
  TextSpan(
    text:
        '  •  App Usage Data: Your interactions with the app, including scenario choices, emotion logs, and progress tracking.\n',
    style: _kBulletStyle,
  ),
  TextSpan(
    text:
        '  •  Text Input: The responses you type during gamified scenarios.\n',
    style: _kBulletStyle,
  ),
  TextSpan(
    text:
        '  •  Audio Input: Optional voice recordings you choose to provide for emotion detection.\n',
    style: _kBulletStyle,
  ),
  TextSpan(
    text:
        '  •  Feedback Data: Interview responses if you participate in the optional feedback session.\n',
    style: _kBulletStyle,
  ),
  TextSpan(text: '\n'),
  TextSpan(text: '6. How will my data be used?\n', style: _kHeadingStyle),
  TextSpan(
    text: 'Your data will be used for research purposes only, including:\n',
    style: _kBodyStyle,
  ),
  TextSpan(
    text:
        '  •  Analysing the usability and effectiveness of the HATI application.\n',
    style: _kBulletStyle,
  ),
  TextSpan(
    text: '  •  Writing academic publications (thesis, presentation).\n',
    style: _kBulletStyle,
  ),
  TextSpan(
    text: '  •  Improving future versions of the application.\n',
    style: _kBulletStyle,
  ),
  TextSpan(text: '\n'),
  TextSpan(text: '7. Will my data be kept private?\n', style: _kHeadingStyle),
  TextSpan(
    text:
        'Yes. The research team will protect your privacy in the following ways:\n',
    style: _kBodyStyle,
  ),
  TextSpan(
    text:
        '  •  Your data will be assigned a unique participant code for anonymisation. Your name will not be stored with your research data.\n',
    style: _kBulletStyle,
  ),
  TextSpan(
    text:
        '  •  All data will be stored on password-protected, encrypted servers accessible only to the core research team.\n',
    style: _kBulletStyle,
  ),
  TextSpan(
    text:
        '  •  This study complies with the Philippine Data Privacy Act of 2012 (RA 10173).\n',
    style: _kBulletStyle,
  ),
  TextSpan(
    text:
        '  •  Data transmission from third-party services used for emotion detection will be encrypted and will not be permanently stored by these services.\n',
    style: _kBulletStyle,
  ),
  TextSpan(
    text:
        '  •  Your personal identity will never be revealed in any publications or presentations. Only grouped or anonymised data will be reported.\n',
    style: _kBulletStyle,
  ),
];

// ─────────────────────────────────────────────────────────────────────────────
// Step 3 — Risks & Support
// ─────────────────────────────────────────────────────────────────────────────

const List<InlineSpan> kConsentRisksSupportSpans = [
  TextSpan(
    text: '8. What are the potential risks of participating?\n',
    style: _kHeadingStyle,
  ),
  TextSpan(
    text: 'The risks are minimal but may include:\n',
    style: _kBodyStyle,
  ),
  TextSpan(
    text:
        '  •  Temporary Discomfort: Reflecting on anxiety-provoking social situations may cause mild emotional discomfort. This is normal and similar to what you might experience in daily life.\n',
    style: _kBulletStyle,
  ),
  TextSpan(
    text:
        '  •  Privacy Risks: As with any digital application, there is a small risk of data breach, though we take extensive precautions to prevent this.\n',
    style: _kBulletStyle,
  ),
  TextSpan(text: '\n'),
  TextSpan(
    text:
        'If you experience significant distress while using the app, you are encouraged to contact the research team using the information provided at the end of this form. You will also be provided with contact information for the university guidance and counselling office and other mental health resources.\n',
    style: _kBodyStyle,
  ),
  TextSpan(text: '\n'),
  TextSpan(
    text: '9. What are the potential benefits of participating?\n',
    style: _kHeadingStyle,
  ),
  TextSpan(
    text:
        'While there is no guarantee of direct benefit, possible benefits include:\n',
    style: _kBodyStyle,
  ),
  TextSpan(
    text: '  •  Greater awareness of your emotional patterns and triggers.\n',
    style: _kBulletStyle,
  ),
  TextSpan(
    text:
        '  •  Learning and practising coping strategies for social anxiety.\n',
    style: _kBulletStyle,
  ),
  TextSpan(
    text:
        '  •  Contributing to research that may help other students with similar experiences.\n',
    style: _kBulletStyle,
  ),
  TextSpan(
    text:
        '  •  Access to a free, private tool for emotional self-monitoring.\n',
    style: _kBulletStyle,
  ),
];

// ─────────────────────────────────────────────────────────────────────────────
// Step 4 — Your Rights
// ─────────────────────────────────────────────────────────────────────────────

const List<InlineSpan> kConsentParticipantRightsSpans = [
  TextSpan(text: '10. Is my participation mandatory?\n', style: _kHeadingStyle),
  TextSpan(
    text:
        'No. Your participation is completely voluntary. You may choose not to participate, or you may withdraw from the study at any time without any penalty, loss of benefits, or negative consequences to your academic standing.\n\n'
        'If you withdraw, any data collected from you will be deleted and not used in the research.\n',
    style: _kBodyStyle,
  ),
  TextSpan(text: '\n'),
  TextSpan(
    text: '11. Who can I contact if I have questions or concerns?\n',
    style: _kHeadingStyle,
  ),
  TextSpan(
    text: 'If you have any questions about this study, please contact:\n',
    style: _kBodyStyle,
  ),
  TextSpan(
    text:
        '  •  Precious Mae J. Taleon — preciousmae.taleon@wvsu.edu.ph · 09275311191\n',
    style: _kBulletStyle,
  ),
  TextSpan(
    text:
        '  •  Jaspher John E. Samalburo — jaspherjohn.samalburo@wvsu.edu.ph\n',
    style: _kBulletStyle,
  ),
  TextSpan(text: '\n'),
  TextSpan(
    text: 'If you experience distress, you can contact:\n',
    style: _kBodyStyle,
  ),
  TextSpan(
    text:
        '  •  National Centre for Mental Health Crisis Hotline\n'
        '       Tel: (02) 989-8727\n'
        '       Cell: (0917) 899-8727\n',
    style: _kBulletStyle,
  ),
];
