...
class _EmotionPageState extends State<EmotionPage> {
  ...
  Future<void> _startScenario() async {
    ...
    try {
      ...
      setState(() {
        _messages.add(
          _ChatMessage(text: "Error starting scenario.", isUser: false),
        );
      });
    } finally {
  ...
  Future<void> _stepScenario({String? text}) async {
    ...
    try {
      final payload = {"session_id": _sessionId, "text": text};
      ...
      setState(() {
        _messages.add(
          _ChatMessage(text: "Error connecting to server.", isUser: false),
        );
      });
      _scrollToBottom();
    } finally {
    ...
    try {
      ...
      setState(() {
        ...
        if (_messages.isNotEmpty &&
            _messages.last.text == "[Voice message sending...]") {
      ...
      setState(() {
        if (_messages.isNotEmpty &&
          ...
          _messages.removeLast();
          _messages.add(
            _ChatMessage(text: "[Voice message failed to send]", isUser: true),
          );
        }
        _messages.add(
          _ChatMessage(text: "Error connecting to server.", isUser: false),
  ...
  Future<void> startRecording() async {
    if (await _record.hasPermission()) {
      ...
      setState(() {
        _messages.add(
          _ChatMessage(text: "Microphone permission denied.", isUser: false),
        );
      });
      _scrollToBottom();
    }
  ...
  Future<void> stopRecording() async {
    ...
    if (path != null) {
      ...
      setState(() {
        _messages.add(
          _ChatMessage(text: "[Voice message sending...]", isUser: true),
        );
      });
      _scrollToBottom();
